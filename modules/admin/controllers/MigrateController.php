...
use app\modules\admin\assets\AppAssets;
use app\modules\admin\assets\AppUtility;
use app\modules\admin\models\MigrationUtility;
use Yii;
use yii\base\Object;
...
//            print_r($model->attributes); exit;
        ...
        $sysInfo = array();
        $sysInfo['serverInfo'] = php_uname(); //服务器系统信息
        $sysInfo['phpdevInfo'] = PHP_VERSION; //php原型版本号
        $sysInfo['hostInfo'] = $_SERVER["HTTP_HOST"]; //当前域名
        $sysInfo['webInfo'] = $_SERVER['SERVER_SOFTWARE']; //web服务器
        $sysInfo['pathInfo'] = Yii::getAlias('@app'); //当前项目路径
        $sysInfo['libVersion'] = date('Y-m-d H:i:s', filemtime(Yii::getAlias('@yii'))); //当前框架版本
        $sysInfo['dsn'] = \Yii::$app->db->dsn;
