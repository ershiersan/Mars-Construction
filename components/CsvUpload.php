 ...
 */
class CsvUpload extends InputWidget
{
    /**
    ...
    public function run()
    {
        ...
        $name_local_file = str_replace('file_qn_code', 'local_file', $name);
        $id_local_file = str_replace('file_qn_code', 'local_file', $id);
        $name_count = str_replace('file_qn_code', 'count', $name);
        $id_count = str_replace('file_qn_code', 'count', $id);
        ...
        $imgHeight = isset($this->options['height']) ? $this->options['height'] : 200;

        $contents = [];
        $contents[] = Html::hiddenInput($name, $value, ['id' => $id]);
        $contents[] = Html::hiddenInput($name_local_file, $value, ['id' => $id_local_file]);
        $contents[] = Html::hiddenInput($name_count, $value, ['id' => $id_count]);
...
/*
        if(!isset($this->options['is-pic']) || $this->options['is-pic'] == 1) {
            $contents[] = Html::tag('img', '', ['id' => $imgId, 'style' => '', 'src' => $value?$url:'', 'width' => $imgWidth, 'height' => $imgHeight, 'style' => $value?'':'display:none;']);
        }
*/
        $contents[] = Html::tag('div', $value, ['id' => $imgId.'tip', 'style'=> 'color:red;margin-top:10px;']);
        $contents[] = Html::endTag('div');
        echo implode("\n", $contents) . "\n";

        $hiddenInputId = $id;
        $objectName = str_replace('-' , '_', $buttonId);
        $uploadUrl = Url::to(['/admin/code-recover/fileupload']);
        $js = <<<EOF
            ...
            {$objectName}.on('success',function(data){ 
                ...
                if(res && res.code == 1) {
                    if(res.data.line == 0) {
                        ...
                        $('#{$id_local_file}').val(res.data.localFile);
                        $('#{$id_count}').val(res.data.line);
                    }
                    $('#{$imgId}tip').html('上传成功！共'+res.data.line+'条记录');
                } else {
                    $('#{$hiddenInputId}').val('');
                    $('#{$id_local_file}').val('');
                    $('#{$id_count}').val('');
                    $('#{$imgId}tip').html(res.msg);
                }
            });
