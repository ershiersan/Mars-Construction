...
class AssetManager extends Component
{
    ...
    /**
     * 如果为true， 会将 静态文件映射信息 打在 log里 Yii::info('file to file', 'AssetManager');
     */
    public $debug = false;
    ...
    protected function publishFile($src)
    {
        ...
        if($this->debug) {
            Yii::info("请将文件{$src}映射到{$this->baseUrl}/{$dir}/{$fileName}", 'AssetManager');   
        }
    ...
    protected function publishDirectory($src, $options)
    {
        ...
        if($this->debug) {
            Yii::info("请将文件{$src}映射到{$this->baseUrl}/{$dir}", 'AssetManager');   
        }
