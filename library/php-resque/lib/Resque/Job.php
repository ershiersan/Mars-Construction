...
class Resque_Job
{
	 ...
	 */
	public function updateStatus($status, $error_times = 0)
	{
		...
		$statusInstance->update($status, $error_times);
	}
	...
	public function fail($exception)
	{
		...
		if(!empty($this->payload['id'])) {
                        $statusInstance = new Resque_Job_Status($this->payload['id']); 
			$error_times = $statusInstance->get('error_times');
			$error_times = empty($error_times) ? 1 : $error_times + 1;

			$this->updateStatus(Resque_Job_Status::STATUS_FAILED, $error_times);
			if($error_times < 5) { // rework can repeat 5 times dingyl
