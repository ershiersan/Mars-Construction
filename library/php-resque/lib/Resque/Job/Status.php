...
class Resque_Job_Status
{
	 ...
	 */
	public function update($status, $error_times=0)
	{
		if(!$this->isTracking()) {
			return;
		}
		...
		// dingyl rework the error job
		if($error_times > 0) {
			$statusPacket['error_times'] = $error_times;
		} else {
			$error_times = $this->get('error_times');
	...
	public function get($key='status')
	{
		...
		return empty($statusPacket[$key]) ? '':$statusPacket[$key];
	}
